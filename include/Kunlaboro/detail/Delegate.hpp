...
private:
	...
	template<typename T>
	using EnableIfNotMemberPair = typename ::std::enable_if < !(is_member_pair<T>::value || is_const_member_pair<T>::value), R > ::type;
	template<typename T>
	using EnableIfMemberPair = typename ::std::enable_if < is_member_pair<T>::value || is_const_member_pair<T>::value, R > ::type;

	template<typename T>
	static EnableIfNotMemberPair<T> functor_stub(void* const object_ptr, A&&... args)
	{
		return (*static_cast<T*>(object_ptr))(::std::forward<A>(args)...);
	}

	template<typename T>
	static EnableIfMemberPair<T> functor_stub(void* const object_ptr, A&&... args)
	{
...
namespace std
{
  ...
  struct hash<::Kunlaboro::detail::Delegate<R (A...)> >
  {
    size_t operator()(::Kunlaboro::detail::Delegate<R (A...)> const& d) const noexcept
    {
      auto const seed(hash<void*>()(d.object_ptr_));

      return hash<typename ::Kunlaboro::detail::Delegate<R (A...)>::stub_ptr_type>()(
